...
export function EditLevel({ initialData }: EditTermProps) {
  const router = useRouter();
  const path = usePathname();
  const params = useParams();

  const levelId : string  = params.levelEditId;
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    try {
      await updateLevel(levelId,values, path);
      router.push(`/admin/${params.adminId}/system-config/manage-levels`);
      form.reset();
      toast({
